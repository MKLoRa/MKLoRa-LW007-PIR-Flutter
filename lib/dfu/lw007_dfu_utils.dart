...
Future<String> lw007PrepareDfuFirmwarePath(PlatformFile picked) async {
  final name = picked.name.trim();
  if (name.isEmpty || !name.toLowerCase().endsWith('.zip')) {
    throw Lw007DfuFileException('File error!');
  }
  ...
  final tempDir = Directory.systemTemp.createTempSync('lw007_dfu_');
  final fileName = name.split('/').last.split('\\').last;
  final target = File('${tempDir.path}/$fileName');
  await target.writeAsBytes(bytes, flush: true);
  debugPrint('[LW007 DFU] firmware copied to ${target.path} (${bytes.length} bytes)');
  return target.path;
}

class Lw007DfuFileException implements Exception {
  Lw007DfuFileException(this.message);
